...
export const initializeProxy = async ({ accountId, aiGatewayContext, blobsContext, command, config, configPath, debug, env: configEnv, geoCountry, geolocationMode, getUpdatedConfig, inspectSettings, mainPort, offline, passthroughPort, projectDir, repositoryRoot, settings, siteInfo, state, deployEnvironment, }) => {
...
const prepareServer = async ({ aiGatewayContext, command, config, configPath, debug, env: configEnv, featureFlags, getUpdatedConfig, inspectSettings, port, projectDir, repositoryRoot, deployEnvironment, }) => {
    try {
